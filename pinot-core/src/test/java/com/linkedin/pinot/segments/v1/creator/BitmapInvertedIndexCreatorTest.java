...
public class BitmapInvertedIndexCreatorTest {
  ...
  public void testMultiValue() throws IOException {
    int[][] dictIds = new int[NUM_DOCS][MAX_NUM_MULTI_VALUES];
    int[] numValues = new int[NUM_DOCS];
    ...
    for (int docId = 0; docId < NUM_DOCS; docId++) {
      ...
      numValues[docId] = numValuesForDoc;
      totalNumValues += numValuesForDoc;
    ...
    try (OnHeapBitmapInvertedIndexCreator onHeapCreator = new OnHeapBitmapInvertedIndexCreator(ON_HEAP_INDEX_DIR,
        COLUMN_NAME, CARDINALITY)) {
      for (int docId = 0; docId < NUM_DOCS; docId++) {
        onHeapCreator.addMV(docId, dictIds[docId], numValues[docId]);
      }
      onHeapCreator.seal();
    }
    ...
    // Generate inverted index using OffHeapBitmapInvertedIndexCreator
    try (OffHeapBitmapInvertedIndexCreator offHeapCreator = new OffHeapBitmapInvertedIndexCreator(OFF_HEAP_INDEX_DIR,
        new DimensionFieldSpec(COLUMN_NAME, DataType.INT, false), CARDINALITY, NUM_DOCS, totalNumValues)) {
      for (int docId = 0; docId < NUM_DOCS; docId++) {
        offHeapCreator.addMV(docId, dictIds[docId], numValues[docId]);
      }
      offHeapCreator.seal();
    }
