...
public class SegmentColumnarIndexCreator implements SegmentCreator {
  ...
  public void init(SegmentGeneratorConfig segmentCreationSpec, SegmentIndexCreationInfo segmentIndexCreationInfo,
    ...
    if (outDir.exists()) {
      throw new RuntimeException("Segment output directory: " + outDir.getAbsolutePath() + " already exists");
    }
    Preconditions.checkState(outDir.mkdirs());
    _indexDir = outDir;
    ...
    Preconditions.checkState(fieldSpecs.size() == indexCreationInfoMap.size());
    ...
    for (FieldSpec fieldSpec : fieldSpecs) {
      ...
      Preconditions.checkNotNull(indexCreationInfo);
  ...
  public void indexRow(GenericRow row) {
    for (final String column : _forwardIndexCreatorMap.keySet()) {
      try {
        ...
        if (schema.getFieldSpecFor(column).isSingleValueField()) {
          if (dictionaryCreator != null) {
            ...
            ((SingleValueForwardIndexCreator) _forwardIndexCreatorMap.get(column)).index(docIdCounter, dictionaryIndex);
            // TODO : {refactor inverted index addition}
            if (_invertedIndexCreatorMap.containsKey(column)) {
              _invertedIndexCreatorMap.get(column).addSV(docIdCounter, dictionaryIndex);
            }
          } else {
            ((SingleValueRawIndexCreator) _forwardIndexCreatorMap.get(column)).index(docIdCounter, columnValueToIndex);
          }
        } else {
          int[] dictionaryIndex = dictionaryCreator.indexOfMV(columnValueToIndex);
          ((MultiValueForwardIndexCreator) _forwardIndexCreatorMap.get(column)).index(docIdCounter, dictionaryIndex);

          // TODO : {refactor inverted index addition}
          if (_invertedIndexCreatorMap.containsKey(column)) {
            _invertedIndexCreatorMap.get(column).addMV(docIdCounter, dictionaryIndex, dictionaryIndex.length);
          }
        }
      } catch (Exception e) {
